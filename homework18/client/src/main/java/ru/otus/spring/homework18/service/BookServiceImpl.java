...
public class BookServiceImpl implements BookService {
    ...
    public Book insert(BookDto bookDto) {
        ...
        ResponseEntity<Book> response = restTemplate.exchange("http://localhost:8081/api/book",
                HttpMethod.POST,
    ...
    public List<Book> showAllRows() {
        ResponseEntity<List<Book>> response = restTemplate.exchange("http://localhost:8081/api/book",
                HttpMethod.GET, null,
    ...
    public Book showById(long id) {
        ResponseEntity<Book> response = restTemplate.getForEntity("http://localhost:8081/api/book?id={id}" + id, Book.class);
        return response.getBody();
    }
    ...
    public String delete(Book book) {
        restTemplate.delete("http://localhost:8081/api/book/id={id}", book.getId());
        return "Success deleting";
    }

    private Book convertToDomain(BookDto bookDto) {
        ResponseEntity<Author> responseAuthor = restTemplate.getForEntity("http://localhost:8081/api/author/name=" + bookDto.getAuthor(), Author.class);
        Author tempAuthor = responseAuthor.getBody();
        ResponseEntity<Genre> responseGenre = restTemplate.getForEntity("http://localhost:8081/api/genre/name=" + bookDto.getGenre(), Genre.class);
        Genre tempGenre = responseGenre.getBody();
        if (tempGenre == null) {
            responseGenre = restTemplate.exchange("http://localhost:8081/api/genre",
                    HttpMethod.POST,
    ...
    public Book showById_Fallback(long id) { return new Book("Error with show book", new Author("null"), new Genre("null"));}

    public String delete_Fallback(Book book) { return "Error with delete book";}
